...
BOOL DlgCombine::OnInitDialog() 
{
	...
	while (pos != NULL) {
		...
		while (pos2 != NULL) {
			CeDMDoc* pDoc = (CeDMDoc*) pTemplate->GetNextDoc(pos2);
			ASSERT(pDoc->IsKindOf(RUNTIME_CLASS(CeDMDoc)));
...
{
	pDocR = (CeDMDoc*) m_imgr.GetItemData(m_imgr.GetCurSel());
	pDocG = (CeDMDoc*) m_imgg.GetItemData(m_imgg.GetCurSel());
	pDocB = (CeDMDoc*) m_imgb.GetItemData(m_imgb.GetCurSel());
	pDocA = (CeDMDoc*) m_imga.GetItemData(m_imga.GetCurSel());
