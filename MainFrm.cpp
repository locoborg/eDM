...
void CMainFrame::OnEditPaste() 
{
	...
	if (NewDoc)	{
		...
		CString s;
		s.Format("Clipboard Image %d",((CeDMApp*)AfxGetApp())->m_nDocCount++);
		NewDoc->SetTitle(s);
...
{ //<<<TWAIN>>>
	...
	if (NewDoc)	{
		...
		CString s;
		s.Format("Acquired Image %d",((CeDMApp*)AfxGetApp())->m_nDocCount++);
		NewDoc->SetTitle(s);
		NewDoc->UpdateAllViews(0,WM_USER_NEWIMAGE);
		NewDoc->UpdateStatusBar();
	}
...
void CMainFrame::OnFileCapture() 
{
	...
	if (dlg.DoModal()==IDOK){
		...
		if (NewDoc)	{
			...
			NewDoc->image = newima;
			CString s;
			s.Format("Captured Image %d",((CeDMApp*)AfxGetApp())->m_nDocCount++);
			NewDoc->SetTitle(s);
			NewDoc->UpdateAllViews(0,WM_USER_NEWIMAGE);
			NewDoc->UpdateStatusBar();
		}
